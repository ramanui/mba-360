(function () {
  ...
  if (entranceSlider) {
    ...
    var initEntranceSlider = function () {
      ...
      Promise.resolve().then(function () {
        // Swiper (incl. Navigation module) already loaded globally via
        // src/vendor/swiper/swiper-bundle.min.js
        var Swiper = window.Swiper;

        new Swiper(entranceSlider, {
          // modules auto-registered by swiper-bundle.min.js
  ...
  const header = document.querySelector(".clg-header");
  if (!header) return;
  const modal = header.querySelector("#videoModal-second");
  const frame = header.querySelector("#videoFrame-second");
  ...
  /* ---------- Close Video ---------- */ function closeVideo() {
    modal.classList.remove("active");
    frame.src = "";
  }
  /* ---------- Close Button ---------- */ const closeBtn =
    modal.querySelector(".videoModal__close");
  if (closeBtn) {
    closeBtn.addEventListener("click", closeVideo);
  }
  /* ---------- Overlay ---------- */ const overlay = modal.querySelector(
    ".videoModal__overlay",
  );
  if (overlay) {
