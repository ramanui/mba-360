(function () {
  ...
  /* ---- 2) Pill Tabs — drag to scroll + center clicked tab ---- */
  document.querySelectorAll(".clg-pillTabs").forEach(function (bar) {
    ...
    /* Click tab → smoothly center it */
    bar.addEventListener("click", function (e) {
      var tab = e.target.closest("a, button");

      if (!tab || !bar.contains(tab)) return;

      var tabCenter = tab.offsetLeft + tab.offsetWidth / 2;
      var barCenter = bar.clientWidth / 2;

      bar.scrollTo({
        left: tabCenter - barCenter,
        behavior: "smooth",
      });
    });
  });
